...
// newSearchRows returns sqlmock columns matching the actual SELECT in SearchPages.
func newSearchRows() *sqlmock.Rows {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockSetup: func(mock sqlmock.Sqlmock) {
				rows := newSearchRows().
					AddRow("https://golang.org", "The Go Programming Language", "Go is an open source programming language...", "Go lang description", 0.95, 0.8, 0.905, time.Now()).
					AddRow("https://go.dev", "Go.dev", "Get started with Go documentation...", "", 0.92, 0.7, 0.846, time.Now()).
					AddRow("https://golang.org/doc", "Documentation", "Learn how to use Go...", "", 0.88, 0.6, 0.682, time.Now())

				mock.ExpectQuery(`seenpages sp`).
		...
		},
		{
			...
			mockSetup: func(mock sqlmock.Sqlmock) {
				rows := newSearchRows().
					AddRow("https://golang.org/doc/tutorial", "Tutorial: Getting started", "In this tutorial you'll learn the basics...", "", 0.95, 0.9, 0.915, time.Now()).
					AddRow("https://tutorial.golang.org", "Go Tutorial", "Complete guide to learning Go...", "", 0.90, 0.85, 0.865, time.Now())
		...
		},
		{
			...
			mockSetup: func(mock sqlmock.Sqlmock) {
				// No query expected — empty tokens return early
			},
			expectedLen:   0,
			expectedError: false,
		},
		{
			...
			mockSetup: func(mock sqlmock.Sqlmock) {
				rows := newSearchRows().
					AddRow("https://test.com", "Test Page", "This is a test page body", "", 0.85, 0.7, 0.745, time.Now())
		...
		},
		{
			...
			mockSetup: func(mock sqlmock.Sqlmock) {
				rows := newSearchRows().
					AddRow("https://test.com", nil, nil, nil, 0.85, 0.7, 0.745, time.Now())
...
func TestHandler_SearchPages_Integration(t *testing.T) {
	...
	rows := newSearchRows().
		AddRow("https://example.com/go/tutorial", "Go Tutorial Complete", "Learn Go programming from scratch...", "Go tutorial description", 0.95, 0.9, 0.915, crawlTime).
		AddRow("https://golang.org", "The Go Programming Language", "Official Go website with documentation...", "", 0.92, 0.8, 0.836, crawlTime).
		AddRow("https://go.dev/learn", "Learn Go", "Interactive Go tutorials and guides...", "", 0.88, 0.75, 0.789, crawlTime)
